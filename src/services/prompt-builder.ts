 ...
 * @param customContext Optional user-provided context from command-line
 * @returns Formatted context prompt string
 */
export function buildContextPrompt(bundle: IntentBundle | null, customContext?: string): string {
    if (!bundle && !customContext) {
        return 'No active AI session context found. Rely solely on the diff.';
    }

    let prompt = '';

    // Add custom context first if provided (highest priority)
    if (customContext) {
    ...
    if (bundle) {
        prompt += `AI CONTEXT (Source: ${bundle.source}, Confidence: ${bundle.confidence.toFixed(2)}):\n\n`;
